...
// FORCED PRODUCTION OVERRIDE: Bypassing Vercel Environment Variables entirely.
const FINAL_BASE_URL = 'https://nishu78-saathishikshak-backend.hf.space/api/v1'
...
export const apiClient = {
  health: () => axiosInstance.get('/health').then(r => r.data),
  verifyAuth: (req: AuthVerifyRequest): Promise<TeacherProfile> => axiosInstance.post('/auth/verify', req).then(r => r.data),
  query: (req: QueryRequest): Promise<QueryResponse> => axiosInstance.post('/query', req).then(r => r.data),
  getSessions: (teacherId: string): Promise<SessionRecord[]> => axiosInstance.get(`/session/${teacherId}`).then(r => r.data),
  submitFeedback: (req: FeedbackRequest): Promise<void> => axiosInstance.post('/feedback', req).then(r => r.data),
  getScenarios: (): Promise<{ scenarios: import('../offline/db').OfflineScenario[]; count: number }> => axiosInstance.get('/scenarios').then(r => r.data),
}